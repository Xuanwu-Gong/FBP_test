...
for i in range(sinogram.shape[1]):
    ...
    tempFT = radon1DFFT[0:256, i].copy()
    radon1DFFT[0:256, i] = radon1DFFT[256:512, i]
    radon1DFFT[256:512, i] = tempFT

    
for j in range(sinogram.shape[1]):
    if (j < 45):
        kuozhan = 1/(math.cos(float(j)/ 180. * np.pi))
    elif ((j >= 45) & (j < 135)):
        kuozhan = 1/(math.sin(float(j)/ 180. * np.pi))
    elif ((j >= 135) & (j < 180)):
        kuozhan = 1/(math.cos(float(180 - j)/ 180. * np.pi))
    for k in range(sinogram.shape[0]):
        
        if ((k < 256) & (j<= 90)):
            dts = float(256. - k)
            x = ( dts * kuozhan * math.cos(float(j)/ 180. * np.pi) + 255. ) 
            y = ( dts * kuozhan * math.sin(float(j)/ 180. * np.pi) + 255. )
            try:
                radonFFT[round(x), round(y)] += radon1DFFT[k, j].copy() 
            except Exception as e:
                print(round(x), round(y), k, j)
                print(e)
            
        elif ((k < 256) & (j > 90)):
            dts = float(256. - k)
            x = ( 256. + dts * kuozhan * math.cos(float(j)/ 180. * np.pi) ) 
            y = ( dts * kuozhan * math.sin(float(j)/ 180. * np.pi) + 255. ) 
            try:
                radonFFT[round(x), round(y)] += radon1DFFT[k, j].copy()
            except Exception as e:
            ...
            dts = float(k - 255.)
            x = ( 256. - dts * kuozhan * math.cos(float(j)/ 180. * np.pi) ) 
            y = ( 256. - dts * kuozhan * math.sin(float(j)/ 180. * np.pi) ) 
            try:
                radonFFT[round(x), round(y)] += radon1DFFT[k, j].copy()
            except Exception as e:
                print(round(x), round(y), k, j)
            ...
            dts = float(k - 255.)
            x = ( 255. - dts * kuozhan * math.cos(float(j)/ 180. * np.pi)  ) 
            y = ( 256. - dts * kuozhan * math.sin(float(j)/ 180. * np.pi)  )
            try:
                radonFFT[round(x), round(y)] += radon1DFFT[k, j].copy()
            except Exception as e:
...
#radonFFT = radonFFT * (np.abs(fft).max() / np.abs(radonFFT).max())
